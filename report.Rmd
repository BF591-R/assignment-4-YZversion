...
caption_4 <- 'Principal-component variance contributions with cumulative variance line for the CRC microarray cohort.'
caption_5 <- 'PC1 vs PC2 biplot showing SixSubtypesClassification groupings from the Marisa et al. dataset.'
caption_8 <- 'Heatmap of FDR < 1% probe intensities scaled per gene using the RdBu color-blind friendly palette.'
```
...
variance_plot <- ggplot(variance_tibble, aes(x = principal_components)) +
    geom_col(aes(y = variance_explained), fill = "#1f78b4", width = 0.7, alpha = 0.85) +
    geom_line(aes(y = cumulative, group = 1), color = "#b22222", size = 0.8) +
    ...
    labs(
        ...
        subtitle = "Bars show individual PCs; line traces cumulative variance"
    ) +
    theme_minimal() +
    theme(
